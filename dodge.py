...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, speed1=5, speed2=20, size1=20, size2=10):
        ...
        # Where the enemy should appear (off screen, anywhere between 20 and 100px right, and anywhere from 100px to 
        # bottom)
        self.rect = self.surf.get_rect(
            center=(
    ...
    # Update where the enermy is. If enemy has passed the left hand side of the screen, enemy is removed and player 
...
while running:
    for event in pygame.event.get():
        if event.type == KEYDOWN:
            ...
            running = False
        # If we have not exceeded the maximum number of enemies, add a new enemy to the enemies and all_sprites groups. 
        # Increment enemies counter
        elif event.type == ADDENEMY:
            if num_enemies < levels[level - 1][4]:
                new_enemy = Enemy(speed1=levels[level - 1][0], speed2=levels[level - 1][1], size1=levels[level - 1][2],
                                  size2=levels[level - 1][3])
    ...
    if play:
        ...
        # If the player has collided with any member of the enemies group, the player is killed and the 'you lose' 
        # message is displayed. Play loop is stopped.
